 ...
 *     `\\57`, `\\u002f`, `\\u{2f}`, `\\x{2f}`) are reconstructed as `/` or `../` plus the
 ...
 *     even inside quotes (`eval 'PWD=;'`) or after a comma; `env -i`, `env -u HOME|PWD`,
 *     and `os.environ.pop`/`del` of those names fail closed. Quote-split `file:` schemes
 *     (`f'ile://...`, `'f'+'ile://...'`) are joined before the URL check. `../` after
